...
/**
 * Detecta los tres tipos de eventos comparando el estado actual de Glide
 * contra lo que ya está en el log de notificaciones.
 *
 * @param {object} input
...
export function detectEvents({
  ...
  for (const row of ocRows) {
    ...
    events.push({
      ...
      payload: {
        ...
        correos: [
          row[OC_COLUMNS.correo0],
          row[OC_COLUMNS.correo1],
          row[OC_COLUMNS.correo2],
        ].filter(Boolean),
      },
    });
  }
  ...
  // c) OPI completo: todos los items de un mismo OPI están en estado
  // "recibido". Se agrupa por número de OPI (ignorando items sin OPI).
