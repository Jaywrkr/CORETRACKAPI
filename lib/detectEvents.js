...
// Nombres de personal en la OC (columnas Personal/Personal 1/Personal 2).
// Se resuelven a email contra la tabla Users en runCheck.js, ya que es la
// fuente confiable (Correo 0/1/2 depende de un workflow nativo que no
// corre siempre).
function ocPersonalNames(ocRow) {
  if (!ocRow) return [];
  return [
    ocRow[OC_COLUMNS.personal],
    ocRow[OC_COLUMNS.personal1],
    ocRow[OC_COLUMNS.personal2],
  ].filter(Boolean);
}
 ...
 *   booleanos como NUEVA_OC/HW_AGREGADO/SW_AGREGADO, el conteo de OC
 *   recibidas para OPI_PROGRESO).
...
export function detectEvents({
  ...
  for (const row of ocRows) {
    ...
    events.push({
      ...
      payload: {
        ...
        personalNames: ocPersonalNames(row),
      },
    });
  }

  // b) Item agregado a una OC existente: toda fila de *HARDWARE dispara
  // HW_AGREGADO, toda fila de *SOFTWARE dispara SW_AGREGADO (son eventos —
  // y correos — separados).
  const allItems = [
  ...
  for (const item of allItems) {
    const eventType =
      item.sourceType === "hardware" ? EVENT_TYPES.HW_AGREGADO : EVENT_TYPES.SW_AGREGADO;
    if (!item.rowId || notifiedCount(item.rowId, eventType) > 0) continue;
    ...
    events.push({
      ...
      payload: {
        ...
        personalNames: ocPersonalNames(ocRow),
      },
    });
  }
  ...
  for (const [opiKey, rows] of ocsByOpi.entries()) {
    ...
    const personalNames = [...new Set(rows.flatMap((row) => ocPersonalNames(row)))];
