...
export async function runCheck() {
  ...
  const [ocRows, hardwareRows, softwareRows, usersRows, notifiedCounts] = await Promise.all([
  ...
  for (const event of events) {
    ...
    try {
      const personalEmails = resolvePersonalEmails(payload.personalNames, userEmailMap);
      const recipients = resolveRecipients(eventType, [
        ...payload.correos,
        ...personalEmails,
      ]);
